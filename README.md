...
I found that Tensorflow optimization tool may not be able to further convert the variables of batch normalization to constants whenever the placeholder is created for is_training. Thus, I constructed a duplicated graph the is_training set to a tf constant and loaded the weights of the original graph so that the optimization works without losing accuracy. I then proceed to freeze, optimize, and fuse the graph for inference deployment. The actual inference engine is names as fused_graph.pb.
